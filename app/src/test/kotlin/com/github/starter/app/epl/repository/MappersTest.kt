...
class MappersTest {
    ...
    fun `eplMatch should handle nulls with defaults`() {
        ...
        whenever(readable.get("wk", Int::class.java)).thenReturn(null)
        whenever(readable.get("matchDate", LocalDate::class.java)).thenReturn(null)
    ...
    fun `field with Class should return null if db value is null`() {
        ...
        val result: String? = mapper.apply(readable)
        assertNull(result)
    }
    ...
    fun `field with Class and defaultValue should map correctly`() {
        val count = 5
        whenever(readable.get("count_val", Int::class.javaObjectType)).thenReturn(count)
